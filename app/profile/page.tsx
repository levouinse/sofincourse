    ...
    <div className="min-h-screen bg-[#0a0f14] cyber-grid">
      ...
      <main className="max-w-4xl mx-auto px-4 py-12">
        <div className="mb-8">
          ...
          <p className="text-gray-500">Manage your account and track your progress</p>
        </div>
        ...
        <div className="grid gap-6">
          <Card className="bg-[#0f1419] border-[#282d35]">
            ...
            <CardContent className="space-y-4">
              <div className="flex items-center gap-4">
                <div className="w-20 h-20 rounded-full bg-[#9bff00]/10 border-2 border-[#9bff00] flex items-center justify-center">
                  <User className="w-10 h-10 text-[#9bff00]" />
                </div>
                <div>
                  <h2 className="text-2xl font-bold text-gray-200">{user?.displayName || 'User'}</h2>
                  <p className="text-gray-500 text-sm font-mono">Member since {user?.metadata?.creationTime ? new Date(user.metadata.creationTime).toLocaleDateString() : 'N/A'}</p>
                </div>
              </div>

              <div className="grid gap-3 pt-4 border-t border-[#282d35]">
                <div className="flex items-center gap-3 text-gray-400">
                  <Mail className="w-5 h-5 text-[#9bff00]" />
                  <span>{user?.email}</span>
                </div>
                <div className="flex items-center gap-3 text-gray-400">
                  <Calendar className="w-5 h-5 text-[#9bff00]" />
                ...
                <div className="flex items-center gap-3">
                  <User className="w-5 h-5 text-[#9bff00]" />
                  <span className="text-gray-400">Role:</span>
                  {role === 'admin' ? (
                    <span className="px-3 py-1 text-xs bg-[#9bff00] text-black rounded font-bold font-mono">
                      ADMIN
                    </span>
                  ) : (
                    <span className="px-3 py-1 text-xs bg-[#282d35] text-gray-400 rounded font-mono">
                      USER
                    </span>
                  )}
                </div>
          ...
          <Card className="bg-[#0f1419] border-[#282d35]">
            ...
            <CardContent>
              <div className="grid grid-cols-2 gap-4">
                <div className="bg-[#0a0f14] border border-[#282d35] rounded-lg p-6 text-center">
                  <Award className="w-8 h-8 text-[#9bff00] mx-auto mb-2" />
                  <div className="text-3xl font-bold text-[#9bff00] mb-1">{stats.completed}</div>
                  <div className="text-sm text-gray-500 font-mono">COMPLETED</div>
                </div>
                <div className="bg-[#0a0f14] border border-[#282d35] rounded-lg p-6 text-center">
                  <BookOpen className="w-8 h-8 text-gray-400 mx-auto mb-2" />
                  <div className="text-3xl font-bold text-gray-400 mb-1">{stats.inProgress}</div>
                  <div className="text-sm text-gray-500 font-mono">IN PROGRESS</div>
                </div>
              </div>
            </CardContent>
          </Card>
          ...
          <Card className="bg-[#0f1419] border-[#282d35]">
            ...
            <CardContent className="space-y-3">
              <Link href="/courses">
                <Button className="w-full bg-[#9bff00] hover:bg-[#7acc00] text-black font-bold">
                  BROWSE COURSES
                </Button>
              </Link>
              <Link href="/skill-tree">
                <Button variant="outline" className="w-full border-[#282d35] text-gray-400 hover:bg-[#9bff00]/10 hover:border-[#9bff00] hover:text-[#9bff00]">
                  VIEW SKILL TREE
                </Button>
              </Link>
            </CardContent>
